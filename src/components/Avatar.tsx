...
const petalIcons = [
  { src: CodingOutlineIcon, alt: 'Coding', label: 'Coding is life' },
  { src: NodeJsIcon, alt: 'Node.js', label: 'Npm install' },
  { src: IonicIcon, alt: 'Ionic', label: 'Ionic start' },
  { src: ReactIcon, alt: 'React', label: 'Dynamic Rendering' },
  { src: SupabaseIcon, alt: 'Supabase', label: 'Activate RLS' }
];
          ...
          {petalIcons.map((icon, i) => (
            <div key={i} className={`petal p${i + 1}`}>
              <div className="petal-content">
                ...
                <div className={`petal-label ${i === activeIndex ? 'visible' : ''}`}>
                  {icon.label}
                </div>
              </div>
            </div>
